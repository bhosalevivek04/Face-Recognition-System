...
from gtts import gTTS
from face_module import extract_face_encoding, compare_face
from utils import speech_to_text, prepare_audio
from db import load_face_db_mongo, upsert_face

app = Flask(__name__)

# Load our persistent face database from MongoDB on startup.
face_db = load_face_db_mongo()
...
def register():
    ...
    Expects form-data with:
      - 'image': image file (JPEG/PNG) containing the person's face.
      - 'audio': audio file (MP3, m4a, or WAV) with the person speaking their name.
    The audio file is saved to a temporary file, processed by prepare_audio and speech_to_text,
    then the face encoding is extracted from the image and stored in MongoDB via upsert_face.
    Allows up to 5 samples per person.
    """
    if 'image' not in request.files or 'audio' not in request.files:
        return jsonify({"error": "Missing image or audio file."}), 400
    ...
    try:
        # Save the audio file to a temporary file using its original extension.
        audio_ext = os.path.splitext(audio_file.filename)[1]
        with tempfile.NamedTemporaryFile(suffix=audio_ext, delete=False) as temp_audio:
            audio_file.save(temp_audio.name)
            temp_audio_path = temp_audio.name

        # Convert the audio file if necessary, then extract the name.
        converted_audio_path = prepare_audio(temp_audio_path)
        name = speech_to_text(converted_audio_path)
        ...
        # Upsert the face record into MongoDB.
        try:
            upsert_face(name, encoding, max_samples=5)
        except ValueError as ve:
            return jsonify({"error": str(ve)}), 400

        # Reload our in-memory database (if needed for recognition endpoint).
        global face_db
        face_db = load_face_db_mongo()
...
def recognize():
    ...
    Expects form-data with:
      - 'image': image file to be recognized.
    Processes the image to extract the face encoding, compares it against the stored
    face database (from MongoDB), and returns the recognized name, matching distance,
    and a base64-encoded MP3 (generated by gTTS) of the recognized name.
    """
    if 'image' not in request.files:
        return jsonify({"error": "Missing image file."}), 400
